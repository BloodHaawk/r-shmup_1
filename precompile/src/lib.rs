...
use amethyst::{
    ...
    renderer::{
        ...
        Factory, Format, GraphBuilder, GraphCreator, Kind, RenderGroupDesc, RenderingSystem, SpriteSheet,
        SubpassBuilder, TextureProcessorSystem,
    },
...
impl<'a, 'b> SystemBundle<'a, 'b> for PrecompiledBundle {
    fn build(self, world: &mut World, builder: &mut DispatcherBuilder<'a, 'b>) -> Result<(), Error> {
        ...
        builder.add_thread_local(
            InputSystemDesc::<MovementBindings>::new(Some(Bindings::load(self.bindings_config_path))).build(world),
        );
